 ...
 * Drives the popup + postMessage handshake for the OAuth flow.
 */
...
export async function connectAppUser(opts: {
  ...
  if (!popup) {
    return { success: false, connectorId, error: "Pop-up bloqueado. Permita pop-ups e tente novamente." };
  }
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      resolve(
        data.success && data.api_key
          ...
          : { success: false, connectorId, error: data.error ?? "Falha no OAuth" },
      );
    };
