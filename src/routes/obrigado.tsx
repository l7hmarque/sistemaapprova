  ...
  head: () => ({
    meta: [
      { title: "Recebemos sua solicitação — Approva" },
      {
        name: "description",
        content:
          "Sua solicitação de demonstração foi recebida. Em até 1 dia útil entraremos em contato.",
      },
...
function ObrigadoPage() {
  ...
  useEffect(() => {
    ...
    try {
      w.gtag?.("event", "generate_lead", { plano });
    } catch {
      /* ignorado: recurso opcional indisponível */
    }
    try {
      w.fbq?.("track", "Lead", { plano });
    } catch {
      /* ignorado: recurso opcional indisponível */
    }
  }, [plano]);
    ...
    <MarketingLayout>
      <section className="mx-auto max-w-2xl px-6 py-24 text-center">
        ...
        <p className="mt-4 text-brand-muted text-lg leading-relaxed">
          ...
          <strong className="text-brand-navy">1 dia útil</strong> para combinar a demonstração e
          liberar seu ambiente de 30 dias.
        </p>
        <p className="mt-6 text-brand-muted text-sm">
          Enquanto isso, dá uma olhada nas{" "}
          <Link to="/contadores" className="text-brand-blue underline underline-offset-4">
            vantagens para escritórios
          </Link>{" "}
          ou nas{" "}
          <Link to="/gestores" className="text-brand-blue underline underline-offset-4">
