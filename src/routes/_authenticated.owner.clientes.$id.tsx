...
export const Route = createFileRoute("/_authenticated/owner/clientes/$id")({
    ...
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <header>
        ...
        <div className="flex gap-2 mt-2">
          ...
          <Badge
            variant={data.org.status === "ativo" ? "default" : "secondary"}
            className="uppercase"
          >
            {data.org.status}
          </Badge>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-5">
            <div className="text-xs uppercase text-muted-foreground">Cotações</div>
            <div className="text-3xl font-display mt-1">{data.cotacoes}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <div className="text-xs uppercase text-muted-foreground">Eventos financeiros</div>
            <div className="text-3xl font-display mt-1">{data.eventos}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <div className="text-xs uppercase text-muted-foreground">Prestações</div>
            <div className="text-3xl font-display mt-1">{data.prestacoes}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm uppercase tracking-wide">Detalhes</CardTitle>
        </CardHeader>
        <CardContent className="text-sm space-y-1">
          <div className="flex justify-between">
            <span className="text-muted-foreground">CNPJ</span>
            <span>{data.org.cnpj || "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Criada em</span>
            <span>{new Date(data.org.criado_em).toLocaleDateString("pt-BR")}</span>
          </div>
          {data.org.trial_ate && (
            <div className="flex justify-between">
              <span className="text-muted-foreground">Trial até</span>
              <span>{new Date(data.org.trial_ate).toLocaleDateString("pt-BR")}</span>
            </div>
          )}
          <div className="flex justify-between">
            <span className="text-muted-foreground">Cobrança</span>
            <span>{data.org.cobranca_externa ? "Externa (manual)" : "Stripe"}</span>
          </div>
        </CardContent>
      </Card>
    </div>
