...
import { gerarOrcamentoNoDrive, gerarMapaComparativoNoDrive } from "@/lib/orcamentos.functions";
...
export const Route = createFileRoute("/_authenticated/orcamentos")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Geração de orçamentos e mapa comparativo no Drive a partir de modelos.",
      },
    ],
  }),
        ...
        <Tabs defaultValue="orcamento" className="w-full">
          <TabsList>
            ...
            <TabsTrigger value="mapa">
              <BarChart3 className="mr-1 h-4 w-4" /> Mapa comparativo
            </TabsTrigger>
...
function useFornecedores() {
  ...
  const recarregar = async () => {
    ...
    if (error) {
      toast.error("Falha ao listar fornecedores: " + error.message);
      return;
    }
...
function apagarRascunhoNomeado(nome: string) {
  try {
    localStorage.removeItem(RASCUNHO_NOMEADO_PREFIX + nome);
  } catch {
    /* noop */
  }
}
...
function NovoOrcamento() {
  ...
  const [resultados, setResultados] = useState<
    Array<{ razao: string; url?: string; erro?: string }>
  >([]);
  ...
  const salvarComoRascunho = () => {
    const nome = window.prompt(
      "Nome do rascunho (para reabrir depois):",
      objeto || "Orçamento em aberto",
    );
    if (!nome?.trim()) return;
  ...
  const onAddItem = () =>
    setItens((x) => [...x, { descricao: "", qtd: 1, unidade: "un", precos: [0, 0, 0] }]);
  ...
  const handleGerar = async () => {
    ...
    if (fornsValidos.length === 0)
      return toast.error("Preencha ao menos 1 fornecedor (razão + CNPJ).");
    ...
    const out = (await Promise.all(tarefas)).filter(Boolean) as Array<{
      razao: string;
      url?: string;
      erro?: string;
    }>;
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            ...
            <Input
              value={entidade.razao}
              onChange={(e) => setEntidade({ ...entidade, razao: e.target.value })}
            />
          </div>
          <div>
          ...
          <div>
            ...
            <Input
              value={entidade.representante}
              onChange={(e) => setEntidade({ ...entidade, representante: e.target.value })}
            />
          </div>
          <div>
            ...
            <Input
              value={entidade.cpf}
              onChange={(e) => setEntidade({ ...entidade, cpf: e.target.value })}
            />
          </div>
          <div>
              ...
              <div key={i} className="space-y-2 rounded-md border p-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updForn(i, { razao_social: e.target.value })}
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updForn(i, { representante_legal: e.target.value })}
                  />
                </div>
                <div>
                  <Label className="text-xs">CPF representante</Label>
                  <Input
                    value={f.cpf_representante}
                    onChange={(e) => updForn(i, { cpf_representante: e.target.value })}
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updForn(i, { validadeDias: Number(e.target.value) || 30 })}
                  />
                </div>
              </div>
            ))}
                ...
                <TableRow key={i}>
                  ...
                  <TableCell>
                    <Input
                      type="number"
                      value={it.qtd}
                      onChange={(e) => updItem(i, { qtd: Number(e.target.value) || 0 })}
                    />
                  </TableCell>
                  <TableCell>
                    <Input
                      value={it.unidade}
                      onChange={(e) => updItem(i, { unidade: e.target.value })}
                    />
                  </TableCell>
                  <TableCell>
                    <Input
                      ...
                      step="0.01"
                      value={it.precos[0]}
                      onChange={(e) => updPreco(i, 0, Number(e.target.value) || 0)}
                    />
                  </TableCell>
                  <TableCell>
                    <Input
                      ...
                      step="0.01"
                      value={it.precos[1]}
                      onChange={(e) => updPreco(i, 1, Number(e.target.value) || 0)}
                    />
                  </TableCell>
                  <TableCell>
                    <Input
                      ...
                      step="0.01"
                      value={it.precos[2]}
                      onChange={(e) => updPreco(i, 2, Number(e.target.value) || 0)}
                    />
                  </TableCell>
                  <TableCell>
                    <Button size="icon" variant="ghost" onClick={() => onDelItem(i)}>
        ...
        {rascunhos.length > 0 && (
          <div className="rounded-md border p-3 text-sm">
            ...
            <div className="mt-2 text-xs text-muted-foreground">
              Dica: o que você está editando agora também é salvo automaticamente e restaurado se a
              página recarregar.
            </div>
          </div>
        )}
...
function NovoMapa() {
  ...
  const [itens, setItens] = useState<ItemMapa[]>([
    { descricao: "", unidade: "un", qtd: 1, precos: [0, 0, 0] },
  ]);
  const [enviando, setEnviando] = useState(false);
  ...
  const onAddItem = () =>
    setItens((x) => [...x, { descricao: "", unidade: "un", qtd: 1, precos: [0, 0, 0] }]);
  const onDelItem = (i: number) => setItens((x) => x.filter((_, k) => k !== i));
  const updItem = (i: number, patch: Partial<ItemMapa>) =>
    setItens((x) => x.map((it, k) => (k === i ? { ...it, ...patch } : it)));
  const updPreco = (i: number, col: 0 | 1 | 2, v: number) =>
    setItens((x) =>
      x.map((it, k) => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Mapa Comparativo (Anexo II)</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
          ...
          {forns.map((f, i) => (
            <div key={i} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-6">
              ...
              <div>
                ...
                <Input
                  value={f.dataEmissao}
                  onChange={(e) => updForn(i, { dataEmissao: e.target.value })}
                  placeholder="dd/mm/aaaa"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updForn(i, { dataValidade: e.target.value })}
                  placeholder="dd/mm/aaaa"
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={f.prazoDias}
                  onChange={(e) => updForn(i, { prazoDias: Number(e.target.value) || 0 })}
                />
              </div>
            </div>
          ))}
...
function PresetsTab() {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle>Novo preset</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div>
            ...
            <Input
              ...
              placeholder="ex: Cesta básica mensal"
            />
          </div>
          <div>
            <Label>Objeto</Label>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Presets salvos ({presets.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {presets.length === 0 && (
              ...
              <li key={p.id} className="flex items-start justify-between rounded-md border p-3">
                <div className="text-sm">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {p.objeto} · {p.itens?.length ?? 0} itens
                  </div>
                </div>
                <Button size="icon" variant="ghost" onClick={() => apagar(p.id)}>
                  <Trash2 className="h-4 w-4" />
                </Button>
              </li>
            ))}
...
function HistoricoTab() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Orçamentos salvos ({lista.length})</CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <TableBody>
            {lista.map((o) => (
              <TableRow key={o.id}>
                ...
                <TableCell>
                  <Button size="icon" variant="ghost" onClick={() => apagar(o.id)}>
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </TableCell>
              </TableRow>
